...
const VaultCard: React.FC<VaultCardProps> = ({ vault, onSelect }) => {
  return (
    <button
      ...
      className="flex flex-col justify-center items-start p-4 gap-2 w-[172px] h-[167px] rounded-2xl bg-[rgba(251,250,249,0.06)] hover:bg-[rgba(251,250,249,0.1)] transition-colors"
    >
      <div className="flex flex-col items-start gap-1 w-[140px]">
        ...
        {vault.isShared ? (
          <div className="mt-0.5 inline-flex items-center rounded-full bg-white/10 text-white/70 px-2 py-0.5 text-[10px] font-medium">
            SHARED WITH ME
          </div>
        ) : null}
        <div className="flex flex-row items-center w-full">
