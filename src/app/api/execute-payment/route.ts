...
export async function POST(req: NextRequest) {
  try {
    ...
    // points ! ! ! ! !!!!
    const pointsAmount = (usdcMinor / BigInt(1_000_000)) * BigInt(10);
    if (pointsAmount > BigInt(0)) {
      void broadcastAwardPoints({ to: owner, amount: pointsAmount })
        .then((res) => {
          if ("error" in res) {
        ...
        .catch((e) => {
          console.error("[points-award] unexpected failure", e);
        });
    }
